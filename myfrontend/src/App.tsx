...
export default function WarehouseInventory() {
  ...
  const loadData = async () => {
    ...
    try {
      const [inventoryRes, transactionsRes] = await Promise.all([
        axios.get(`${BACKEND_URL}/api/transactions`, {
          headers: {
        ...
        // fetch(`${BACKEND_URL}/api/transaction-details`),
      ])
  ...
  const getStockStatus = (product: Inventory) => {
                ...
                <TableBody>
                  {inventory.map((item) => (
                    <TableRow key={item.id}>
                      <TableCell className="font-medium text-black">{item.sku}</TableCell>
